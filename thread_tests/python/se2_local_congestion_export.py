"""
se2_local_congestion_export.py

SE2 local-wind congestion interaction export:
- d_congested = 1 if |P_SE2 - P_SE3| > threshold
- wind_cong   = wind_log * d_congested
"""
...
OUT_DIR = os.path.join(BASE_DIR, "stata_input", "local_congestion")
...
def main() -> None:
    ...
    df_proc_reset = df_proc_reset.merge(
        aux[["Timestamp", "occurrence", "d_congested"]],
        on=["Timestamp", "occurrence"],
        how="left",
    )
    df_proc_reset["wind_cong"] = (
        df_proc_reset["Wind_Forecast_Log"] * df_proc_reset["d_congested"]
    )
    ...
    out = df_proc_reset[
        ["Timestamp", "Price_DS", "Wind_Forecast_Log", "Consumption_Log_Deseasonalized",
         "d_congested", "wind_cong"] + netexch_cols
    ].copy()
    out = out.rename(
        columns={
            "Timestamp": "timestamp",
    ...
    out_path = os.path.join(
        OUT_DIR, f"local_congestion_SE2_{start_str}_{end_str}_log.csv"
    )
