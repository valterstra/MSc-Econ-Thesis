"""
plot_structural_break_quarterly.py

Plots the rolling wind coefficient (mean equation, β) from the quarterly
GARCH-X results with piecewise-linear structural breaks overlaid.

Source files:  output from stata/rolling_window_results/
                   rolling_garch_{zone}_1yr_quarterly_hetfull.csv
Output files:  output from stata/structural_break/
                   sb_quarterly_{zone}.png   (one per zone)
...
Usage:
    python plot_structural_break_quarterly.py
"""
...
MAX_BREAKS = 2   # upper limit; BIC chooses 0 … MAX_BREAKS
MIN_SEG    = 6   # minimum windows per segment
...
def load_zone(zone: str) -> pd.DataFrame:
    ...
    path = os.path.join(RESULTS_DIR,
                        f'rolling_garch_{zone}_1yr_quarterly_hetfull.csv')
    raw  = pd.read_csv(path)
    raw['start_date'] = pd.to_datetime(raw['start_date'])

    sub = raw[(raw['type'] == 'coef') & (raw['variable'] == 'wind_log')].copy()
    ...
    # Patch anomalous CI spikes (average of neighbours)
    patches = []
    if zone in ('SE3', 'SE4'):
    ...
    for ts in patches:
        mask = sub['start_date'] == pd.Timestamp(ts)
        idx  = sub.index[mask]
        if len(idx) == 1:
            i = idx[0]
            if 0 < i < len(sub) - 1:
                sub.loc[i, 'se_wind'] = (sub.loc[i - 1, 'se_wind'] +
                                          sub.loc[i + 1, 'se_wind']) / 2
    return sub
...
def select_breaks(t: np.ndarray, b: np.ndarray,
    ...
    for i in range(len(models) - 1):
        ...
        df_num = 2                        # each added break adds 2 params
        df_den = n - 2 * (m1['n'] + 1)
...
def plot_zone(zone: str) -> None:
    ...
    t     = np.arange(n, dtype=float)   # integer time index
    ...
    # Coefficient series + CI band
    ax.plot(dates, b, color=color, linewidth=2, marker='o', markersize=4,
            label=zone, zorder=3)
    ...
    # Segment boundaries (0 … n, as indices)
    boundaries = [0] + result['break_indices'] + [n]

    # Draw fitted trend line for each segment
    legend_trend = True
    ...
    # Vertical break lines + date labels
    ylo, yhi = ax.get_ylim()
    ...
    ax.set_title(
        f'{zone} — Rolling wind coefficient with {n_brk} structural '
        f'break{"s" if n_brk != 1 else ""} (BIC-selected, piecewise linear)',
        fontsize=11)
    ax.set_xlabel('Window midpoint', fontsize=11)
    ax.set_ylabel(r'$\hat{\beta}_{wind}$  (log-price per log-wind unit)',
                  fontsize=11)
    ...
    out_path = os.path.join(OUT_DIR, f'sb_quarterly_{zone}.png')
