"""
rolling_window_export_monthly.py

Preprocesses overlapping 1-year windows stepped by 1 month for each of the
...
Output (per zone × window):
    stata_input/rolling_windows_monthly/armax_input_{ZONE}_{start}_{end}_{pipeline}.csv
    stata_input/rolling_windows_monthly/armax_meta_{ZONE}_{start}_{end}_{pipeline}.csv

Usage:
    python rolling_window_export_monthly.py
"""
...
TEST_RUN  = False   # ← set False to run all 121 windows
...
# Generate windows: 1-year length, monthly step, starting 2015-01-01
...
OUT_DIR  = os.path.join(BASE_DIR, 'stata_input', 'rolling_windows_monthly')
...
if __name__ == '__main__':
    ...
    print(f"Monthly rolling window export: {len(ZONES)} zones × {len(WINDOWS)} windows = {total} exports")
    print(f"Window length: 1 year  |  Step: 1 month")
