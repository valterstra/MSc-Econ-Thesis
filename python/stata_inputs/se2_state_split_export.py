"""
se2_state_split_export.py

SE2 state-split export:
- abs_spread_se2_se3 = |P_SE2 - P_SE3| from raw spot prices
- high_sep = 1 if abs_spread_se2_se3 > threshold
"""
...
SPREAD_THRESHOLD_EUR = float(os.getenv("SPREAD_THRESHOLD_EUR", "5.0"))

BASE_DIR = str(PROJECT_ROOT)
OUT_DIR = os.path.join(BASE_DIR, "stata_input", "state_split")
...
def _read_combined(zone: str) -> pd.DataFrame:
...
def main() -> None:
    start_str = START_DATE.strftime("%Y-%m-%d")
    end_str = END_DATE.strftime("%Y-%m-%d")
    threshold_tag = str(SPREAD_THRESHOLD_EUR).replace(".", "p")
    ...
    se2 = _read_combined("SE2").rename(columns={"Spot_Price": "price_se2_raw"})
    se3 = _read_combined("SE3").rename(columns={"Spot_Price": "price_se3_raw"})
    aux = se2.merge(se3, on=["Timestamp", "occurrence"], how="inner")
    aux["abs_spread_se2_se3"] = (aux["price_se2_raw"] - aux["price_se3_raw"]).abs()
    aux["high_sep"] = (aux["abs_spread_se2_se3"] > SPREAD_THRESHOLD_EUR).astype(int)
    ...
    df_proc_reset = df_proc_reset.merge(
        aux[["Timestamp", "occurrence", "abs_spread_se2_se3", "high_sep"]],
    ...
    out_path = os.path.join(
        OUT_DIR,
        f"state_split_SE2_{start_str}_{end_str}_thr{threshold_tag}_log.csv",
    )
