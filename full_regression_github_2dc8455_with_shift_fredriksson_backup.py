...
def load_commodities(path, start_date='2020-01-01', end_date='2025-12-31'):
    """
    ...
    # Filter to requested analysis range
    df_commodities = df_commodities[
        (df_commodities['Date'] >= pd.to_datetime(start_date)) &
        (df_commodities['Date'] <= pd.to_datetime(end_date))
    ]
...
def load_all_thesis_data(paths, zone_price='SE1', zone_wind='S1', zone_hydro='SE1',
                         zone_exch='SE1', zone_cons='SE1', use_interpolation=False,
                         start_date='2020-01-01', end_date='2025-12-31'):
    """Loads and merges all master files based on standardized Timestamps."""

    # Newer project structure: use pre-combined regional file if available.
    if 'combined' in paths:
        print("\n--- LOADING PRE-COMBINED REGIONAL DATA (2015-2025) ---")
        ...
        # Hydro is still merged from master hydro file to keep variable definition unchanged.
        ...
        # Legacy project structure: separate source files per variable.
    ...
    # Restrict analysis window explicitly (inclusive bounds).
    final_df = final_df[
        (final_df['Datetime'] >= pd.to_datetime(start_date)) &
        (final_df['Datetime'] <= pd.to_datetime(end_date))
    ]

    # Load and merge commodity prices (daily -> hourly) if path provided
    if 'commodities' in paths:
        df_commodities = load_commodities(paths['commodities'], start_date=start_date, end_date=end_date)
        ...
        # Merge gas and (fallback) oil from daily commodity file
        final_df = pd.merge(final_df, df_commodities, on='Date', how='left')

        # Prefer hourly Light Crude Oil file when available
        if 'crude_oil' in paths:
            df_crude = pd.read_excel(paths['crude_oil'])
            df_crude_subset = pd.DataFrame({
                'Datetime': pd.to_datetime(df_crude['Timestamp']),
                'Oil_Price_Hourly': pd.to_numeric(df_crude['Close'], errors='coerce')
            })
            df_crude_subset = df_crude_subset[
                (df_crude_subset['Datetime'] >= pd.to_datetime(start_date)) &
                (df_crude_subset['Datetime'] <= pd.to_datetime(end_date))
            ]
            final_df = pd.merge(final_df, df_crude_subset, on='Datetime', how='left')
            final_df['Oil_Price'] = final_df['Oil_Price_Hourly'].combine_first(final_df['Oil_Price'])
            final_df = final_df.drop(columns=['Oil_Price_Hourly'])
...
def handle_negative_prices(df, method='shift', floor=0.01):
    """
    Check core variables for negative values and handle Price for log safety.

    Methods for Price:
    - 'shift': shift entire Price series so minimum becomes `floor`
    - 'clip': clip values below `floor` up to `floor`
    """
    print("\n" + "="*80)
    print("NEGATIVE VALUE CHECK & PRICE HANDLING")
    print("="*80)

    variables_to_check = ['Price', 'Wind_Forecast', 'Hydro_Reserves', 'Consumption', 'Oil_Price', 'Gas_Price']
    ...
    if not found_negatives:
        print("  [OK] No negative values found in checked variables")
    print("\n  Net_Exchange: NOT CHECKED (expected to have negative values)")

    if 'Price' not in df.columns:
        print("\nPrice column not found. Skipping price handling.")
        print("="*80 + "\n")
        return df

    df_clean = df.copy()
    min_price = df_clean['Price'].min()
    below_floor = (df_clean['Price'] < floor).sum()

    print("\n" + "-"*80)
    print("HANDLING PRICE VARIABLE")
    print("-"*80)
    print(f"Method: {method.upper()} | Floor: {floor}")
    print(f"Price minimum before handling: {min_price:.4f}")
    print(f"Observations below floor: {below_floor}")

    if method == 'shift':
        if min_price < floor:
            shift_amount = floor - min_price
            df_clean['Price'] = df_clean['Price'] + shift_amount
            print(f"SHIFT METHOD: shifted full Price series by {shift_amount:.4f}")
            print(f"New minimum: {df_clean['Price'].min():.4f}")
        else:
            print("SHIFT METHOD: no shift needed (minimum already above floor)")
    elif method == 'clip':
        df_clean['Price'] = df_clean['Price'].clip(lower=floor)
        print(f"CLIP METHOD: clipped {below_floor} observations to {floor}")
    else:
        raise ValueError(f"Unknown method '{method}'. Use 'shift' or 'clip'.")
...
if __name__ == "__main__":
    ...
    # Outlier handling lock for this alignment pass:
    # - Method: Fredriksson
    # - Timing: Late only (after log + deseasonalization)
    OUTLIER_METHOD = 'fredriksson'

    # --- NEGATIVE PRICE HANDLING ---
    # Align with newest file behavior: shift full Price series so min becomes 0.01
    NEGATIVE_PRICE_HANDLING = 'shift'  # 'shift' or 'clip'
    NEGATIVE_PRICE_FLOOR = 0.01
    ...
    # Updated paths for current 2015-2025 data structure.
    # Keep regression specification from this Jan-25 file; only data sources are modernized.
    PATHS = {
    ...
    try:
        # Load and clean dataset for the requested sample window (inclusive)
        data = load_all_thesis_data(
            PATHS,
            zone_price=ACTIVE_ZONE,
            zone_hydro=ACTIVE_ZONE,
            zone_exch=ACTIVE_ZONE,
            zone_cons=ACTIVE_ZONE,
        ...
        # --- STEP 2: HANDLE NEGATIVE PRICES (before log) ---
        data = handle_negative_prices(
            data,
            method=NEGATIVE_PRICE_HANDLING,
            floor=NEGATIVE_PRICE_FLOOR
        )
        ...
        # --- STEP 5: OUTLIER HANDLING (late only, Fredriksson) ---
        if HANDLE_OUTLIERS:
            data, outlier_stats = handle_outliers_fredriksson(
                data,
